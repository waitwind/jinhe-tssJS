;(function($, factory) {
	...
	TreeNode = function(nodeInfo, parent) {
		...
		this.state = parseInt(nodeInfo[_TREE_NODE_STATE]);  // 状态： 停用/启用  1/0
		this.checkState = parseInt(nodeInfo[_TREE_NODE_CHECK_STATE]); /* 节点的选择状态 */
	...
	Tree = function(el, data) {
		...
		this.loadXML = function(node, parent) {
			var childNodes = node.childNodes;
			$.each(data.childNodes, function(i, childNode) {
				...
				this.loadXML(childNode, treeNode);
			});
		};
