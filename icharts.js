$(function(){
 ...
 (function(window) {
    ...
    var iChart_ = (function(window) {
        ...
        bind = function(fn) {
            bindReady();
            if (isReady) fn.call(document, _);
            else readyList.push(function() {
                return fn.call(this);
            });
        },
        _ = function(selector) {
        ...
        _.apply = function(d, e) {
            if (d && e && typeof e == "object") {
                for (var a in e) {
                    if (typeof e[a] != 'undefined') d[a] = e[a]
                }
            }
            if (!e && d) {
                var clone = {};
        ...
        _.applyIf = function(d, e) {
            if (d && _.isObject(e)) {
                for (var a in e) {
                    if (_.isDefined(e[a]) && !_.isDefined(d[a])) d[a] = e[a]
                }
            }
            if (!e && d) {
                return _.apply(d);
        ...
        _.clone = function(a, e, deep) {
            ...
            if (_.isArray(a) && _.isObject(e)) {
                for (var i = 0; i < a.length; i++) {
                    if (deep && _.isObject(e[a[i]])) d[a[i]] = _.clone(e[a[i]], deep);
                    else d[a[i]] = e[a[i]];
                }
            } else if (_.isObject(a)) {
                for (var b in a) {
                    // avoid recursion reference
                    if (e && _.isObject(a[b]) && !(a[b].ICHARTJS_OBJECT)) d[b] = _.clone(a[b], e);
                    else d[b] = a[b];
                }
            }
            return d;
        ...
        _.extend = function() {
            ...
            return function(G, O) {
                ...
                J.plugin = function(M, F) {
                    if (_.isString(M) && _.isFunction(F)) J.plugin_[M] = F;
                };
                return J;
            }
        } ();
        ...
        toRgb = function(color) {
            ...
            if (/^#(([a-fA-F0-9]{6,7})|([a-fA-F0-9]{3}))$/.exec(color)) return hex2Rgb(color);
        ...
        _.apply(_, {
            ...
            DefineAbstract: function(M, H) {
                if (!H[M]) throw new Error("Cannot instantiate the type '" + H.type + "'.you must implements it with method '" + M + "'.");
            },
            getAA: function(tf) {
                if (tf == 'linear') return arithmetic.Linear;
                if (tf == 'easeInOut' || tf == 'easeIn' || tf == 'easeOut') return arithmetic.Cubic[tf];
            ...
            plugin: function(t, m, f) {
                if (_.isFunction(t)) t.plugin(m, f);
            },
            parsePadding: function(s, d) {
                s = s || 0;
                if (_.isNumber(s)) return new Array(s, s, s, s);
            ...
            parseFloat: function(v, d) {
                if (!_.isNumber(v)) {
                    v = pF(v);
                    if (!_.isNumber(v)) throw new Error("[" + d + "]=" + v + "is not a valid number.");
                }
                return v;
            },
            ceil: function(max) {
        ...
        _.Assert = {
            isTrue: function(v, cause) {
                if (v !== true) throw new Error(cause);
            }
        };
        /**
        ...
        _.Event = {
            addEvent: function(ele, type, fn, useCapture) {
                if (ele.addEventListener) ele.addEventListener(type, fn, useCapture);
 ...
 */
    ...
    $.Element.prototype = {
        ...
        fireEvent: function(socpe, name, args) {
            ...
            if (L == 1) return this.events[name][0].apply(socpe, args);
            var r = true;
            for (var i = 0; i < L; i++) {
                if (!this.events[name][i].apply(socpe, args)) r = false;
            }
            return r;
        },
        on: function(n, fn) {
        ...
        set: function(c) {
            if ($.isObject(c)) $.merge(this.options, c);
        },
        pushIf: function(n, v) {
 ...
 */
     ...
     */
        ...
        doConfig: function() {
            ...
            var _ = this._(),
            p = $.parsePadding(_.get('padding')),
            b = _.get('border.enable'),
            b = b ? $.parsePadding(_.get('border.width')) : [0, 0, 0, 0],
            bg = $.toRgb(_.get('background_color')),
            f = _.get('color_factor'),
            g = _.get('gradient') ? 0 : null;
 ...
 */
    $.Html = $.extend($.Element, {
        ...
        css: function(k, v) {
            if ($.isString(k)) if ($.isDefined(v)) this.dom.style[k] = v;
            else return this.dom.style[k];
        },
        applyStyle: function() {
            var styles = this.get('style').split(";"),
            style;
            ...
            if (_.get('showType') == 'follow') {
                ...
                function(c, e, m) {
                    if (_.T.variable.event.mouseover) {
                        setTimeout(function() {
                            if (_.T.variable.event.mouseover) _.follow(e, m, _);
                        },
                        _.get('delay'));
                    }
                });
 ...
 */
    $.Legend = $.extend($.Component, {
        ...
        doConfig: function() {
            ...
            var _ = this._(),
            g = _.root,
            c = $.isNumber(_.get('column')),
            r = $.isNumber(_.get('row')),
            L = _.data.length;
            /**
         ...
         */
            ...
            if (!c && !r) c = _.push('column', 1);
            if (c && !r) r = _.push('row', Math.ceil(L / _.get('column')));
            if (!c && r) c = _.push('column', Math.ceil(L / _.get('row')));
 ...
 */
    ...
    $.Text = $.extend($.Component, {
        ...
        doDraw: function(_) {
            if (_.get('box_feature')) _.T.box(_.x, _.y, _.get(_.W), _.get(_.H), _.get('border'), _.get('f_color'));
 ...
 */
    ; (function($) {
        ...
        parse = function(c, _) {
            ...
            if (_.dataType == 'simple') {
                ...
                var L = g.length,
                item, T, r, stack = _.dataType == 'stacked';
                if (L == 0) {
                    L = c[0].value.length;
                    for (var i = 0; i < L; i++) g.push("");
                }
                _.columns = [];
                for (var i = 0; i < L; i++) {
                    item = [],
     ...
     */
        function Cans(c) {
            ...
            if (!c || !c['tagName'] || c['tagName'].toLowerCase() != 'canvas') throw new Error("there not a canvas element");
        ...
        Cans.prototype = {
            ...
            css: function(a, s) {
                if ($.isDefined(s)) this.canvas.style[a] = s;
                else return this.canvas.style[a];
            },
            /**
         * draw ellipse API
         */
         ...
         */
            sector: function(x, y, r, dw, s, e, c, b, bw, bc, sw, ccw, a2a, font) {
                if (sw) this.arc(x, y, r, dw, s, e, c, b, bw, bc, sw, ccw, !a2a, !font);
            ...
            sector3D: function() {
                ...
                layerPaint = function(x, y, a, b, s, e, ccw, h, c) {
                    ...
                    q2 = $.quadrantd(e);
                    c = $.dark(c);
                    if (q1 == 1 || q1 == 2) layerDraw.call(this, x, y, a, b, ccw, h, s, c);
                    if (q2 == 0 || q2 == 3) layerDraw.call(this, x, y, a, b, ccw, h, e, c);
                };
            ...
            strokeStyle: function(b, w, c, j) {
                if (b) {
                    if (w) this.c.lineWidth = w;
                    if (c) this.c.strokeStyle = c;
                    if (j) this.c.lineJoin = j;
                }
                return this;
            },
            globalAlpha: function(v) {
                if (v) this.c.globalAlpha = v;
                return this;
            },
            fillStyle: function(c) {
                if (c) this.c.fillStyle = c;
                return this;
            },
            arc2: function(x, y, r, s, e, c) {
                if (r) this.c.arc(x, y, r, s, e, c);
                return this;
            },
            ...
            textBaseline: function(l) {
                if (l) this.c.textBaseline = l;
                return this;
            },
            textFont: function(font) {
                if (font) this.c.font = font;
                return this;
            },
            shadowOn: function(s) {
                if (s) {
            ...
            avgLinearGradient: function(xs, ys, xe, ye, c) {
                ...
                for (var i = 0; i < c.length; i++) g.addColorStop(i / (c.length - 1), c[i]);
                return g;
            },
            ...
            fillText: function(t, x, y, max, color, mode, h, sw, ro) {
                ...
                T.each(function(t, i) {
                    try {
                        if (max) this.c.fillText(t, 0, i * h, max);
                        else this.c.fillText(t, 0, i * h);
                    } catch(e) {
         ...
         */
             ...
             */
                if (sw) {
                    this.polygon(bg, b, bw, bc, sw, false, [{
                        x: x,
                        y: y
                    },
                    {
             ...
             */
                this.polygon($.dark(bg), b, bw, bc, false, false, [{
                    x: x,
                    y: y
                },
                {
                ...
                this.polygon(bg, b, bw, bc, false, false, [{
                    x: x,
                    y: y
                },
                {
             ...
             */
                ...
                while (styles.length < 6) styles.push(false);
                ...
                if (rotatey < 0) {
                    if ($.isObject(styles[4])) side.push($.applyIf({
                        points: [{
                            x: x,
                            y: y - h
                        },
                    ...
                    if ($.isObject(styles[0])) side.push($.applyIf({
                        points: [{
                            x: x,
                            y: y
                        },
                ...
                if ($.isObject(styles[1])) side.push($.applyIf({
                    points: [{
                        x: rotatex,
                        y: rotatey
                    },
                ...
                if ($.isObject(styles[2])) side.push($.applyIf({
                    points: [{
                        x: x,
                        y: y
                    },
                ...
                if ($.isObject(styles[3])) side.push($.applyIf({
                    points: [{
                        x: x + w,
                        y: y
                    },
                ...
                if ($.isObject(styles[5])) side.push($.applyIf({
                    points: [{
                        x: x,
                        y: y
                    },
            ...
            polygon: function(bg, b, bw, bc, sw, alpham, p, smooth, smo, l) {
                ...
                if (smooth) {
                    ...
                    for (var i = 1; i < p.length; i++) this.bezierCurveTo(getCurvePoint(p, p[i], i, smo));
                    this.lineTo(fd(bw, l[1].x), fd(bw, l[1].y));
                } else {
                    for (var i = 1; i < p.length; i++) this.lineTo(fd(bw, p[i].x), fd(bw, p[i].y));
                }
            ...
            label: function(p, w, c) {
                return this.save().beginPath().strokeStyle(true, w, c).moveTo(fd(w, p[0].x), fd(w, p[0].y)).bezierCurveTo([p[1].x, p[1].y, p[2].x, p[2].y, p[3].x, p[3].y]).stroke(true).restore();
            },
     ...
     */
         ...
         */
            ...
            segmentRect: function() {
                if (!this.Combination) this.T.clearRect(this.get('l_originx'), this.get('t_originy'), this.get('client_width'), this.get('client_height'));
            },
            resetCanvas: function() {
                if (!this.Combination) this.T.box(this.get('l_originx'), this.get('t_originy'), this.get('client_width'), this.get('client_height'), 0, this.get('f_color'), 0, 0, true);
            },
             ...
             * draw coordinate
             */
                if (_.coo && !_.ILLUSIVE_COO) _.coo.draw();
            ...
            runAnimation: function(_) {
                ...
                if (!_.A_draw) _.variable.animation = {
                    type: 0,
                    time: 0,
                    queue: []
                }
         ...
         */
            ...
            initialize: function() {
                ...
                r = _.get('render');
                if (_.Combination) {
                    $.apply(_.options, $.clone([_.W, _.H, 'padding', 'border', 'client_height', 'client_width', 'minDistance', 'maxDistance', 'minstr', 'centerx', 'centery', 'l_originx', 'r_originx', 't_originy', 'b_originy'], _.root.options, true));
                    _.width = _.get(_.W);
                    _.height = _.get(_.H);
         ...
         */
            oneWay: function(_) {
                var E = _.variable.event,
                comb = _.Combination,
                tot = !_.get('turn_off_touchmove') && !comb,
                mCSS = !$.touch && _.get('default_mouseover_css') && !comb,
                O,
                AO,
                events = $.touch ? ['touchstart', 'touchmove'] : ['click', 'mousemove'];
                _.stopEvent = false;
             ...
             */
                if (!comb) {
                    events.each(function(it) {
                        _.T.addEvent(it,
                        function(e) {
                            if (_.processAnimation || _.stopEvent) return;
         ...
         */
            ...
            doActing: function(_, d, o, i, t) {
                ...
                v = _.getPercent(d.value, d.total);
                /**
             ...
             */
                ...
                if (!_.Combination) {
                    ...
                    w = _.push('client_width', (_.width - _.get('hpadding'))),
                    h;
 ...
 */
    ...
    $.Custom = $.extend($.Component, {
        ...
        isEventValid: function(e, _) {
            if ($.isFunction(this.get('eventValid'))) return this.get('eventValid').call(this, e, _);
            return {
                valid: false
            };
        },
 ...
 */
     ...
     */
        doDraw: function(_) {
            if (_.get('scale_enable')) _.items.each(function(item) {
        ...
        doConfig: function() {
            ...
            var _ = this._(),
            abs = Math.abs,
            customL = _.get('labels').length,
            min_s = _.get('min_scale'),
            max_s = _.get('max_scale'),
            s_space = _.get('scale_space'),
            e_scale = _.get('end_scale'),
            start_scale = _.get('start_scale');
             ...
             */
                if (!s_space || s_space > (e_scale - start_scale)) {
                    var W = ((e_scale - start_scale) + "").indexOf('.') + 1,
                    M = 1;
                    while (W > 0) {
                        W--;
                        M *= 10;
         ...
         */
             ...
             */
                if (_.get('label')) _.labels.push(new $.Text($.applyIf($.apply(_.get('label'), $.merge({
                    text: text,
                    x: x,
                    y: y,
                    originx: x + tx,
         ...
         */
            ...
            if ($.isArray(scale)) {
                 ...
                 */
                    ...
                    if (!s.start_scale) s.min_scale = _.get('minValue');
                    if (!s.end_scale) s.max_scale = _.get('maxValue');
                });
            } else {
    ...
    $.Coordinate2D = $.extend($.Component, {
        ...
        doDraw: function(_) {
            ...
            if (_.get('striped')) {
                ...
                c = $.dark(_.get('background_color'), _.get('striped_factor'), 0);
            }
            var v = (_.get('striped_direction') == 'v');
            _.gridlines.each(function(g, i) {
                if (_.get('striped')) {
                    if (f) {
                        if (v) _.T.box(g.x1, g.y1 + g.width, g.x2 - g.x1, y - g.y1 - g.width, 0, c);
                        else _.T.box(x + g.width, g.y2, g.x1 - x, g.y1 - g.y2, 0, c);
                    }
                    x = g.x1;
                    y = g.y1;
                    f = !f;
         ...
         */
            ...
            if (_.get('axis.enable')) {
                ...
                if (!$.isArray(aw)) _.push('axis.width', [aw, aw, aw, aw]);
            } else {
            ...
            var jp, cg = !!(_.get('gridlinesVisible') && _.get('grids')),
            hg = cg && !!_.get('grids.horizontal'),
            vg = cg && !!_.get('grids.vertical'),
            h = _.get(_.H),
            w = _.get(_.W),
            vw = _.get('valid_width'),
            vh = _.get('valid_height'),
            k2g = _.get('gridlinesVisible') && _.get('scale2grid') && !(hg && vg),
            sw = (w - vw) / 2,
            sh = (h - vh) / 2,
            axis = _.get('axis.width');
            ...
            if (!$.isArray(_.get('scale'))) {
                if ($.isObject(_.get('scale'))) _.push('scale', [_.get('scale')]);
                else _.push('scale', []);
            }
            ...
            if (k2g) {
                 ...
                 */
                    ...
                    scale.items.each(function(item) {
                        if (iol) _.gridlines.push($.applyIf({
 ...
 */
    $.Coordinate3D = $.extend($.Coordinate2D, {
         ...
         * bottom
         */
            if (_.get('pedestal_height')) _.T.cube3D(_.x, _.y + h + _.get('pedestal_height'), xa, ya, false, w, _.get('pedestal_height'), zh * 3 / 2, _.get('axis.enable'), _.get('axis.width'), _.get('axis.color'), _.get('bottom_style'));
            /**
         * board_style
         */
            if (_.get('board_deep')) _.T.cube3D(_.x + offx, _.y + h - offy, xa, ya, false, w, h, _.get('board_deep'), _.get('axis.enable'), _.get('axis.width'), _.get('axis.color'), _.get('board_style'));
            ...
            _.gridlines.each(function(g) {
                if (g.solid) {
                    if (_.get('left_board')) _.T.line(g.x1, g.y1, g.x1 + offx, g.y1 - offy, g.width, g.color);
                    _.T.line(g.x1 + offx, g.y1 - offy, g.x2 + offx, g.y2 - offy, g.width, g.color);
                } else {
                    if (_.get('left_board')) _.T.dotted(g.x1, g.y1, g.x1 + offx, g.y1 - offy, g.width, g.color, g.size, g.fator);
 ...
 */
    ...
    $.Rectangle2D = $.extend($.Rectangle, {
        ...
        doConfig: function() {
            ...
            var _ = this._(),
            tipAlign = _.get('tipAlign');
            if (tipAlign == _.L || tipAlign == _.R) {
                _.tipY = function(w, h) {
                    return _.get('centery') - h / 2;
                };
            } else {
                _.tipX = function(w, h) {
                    return _.get('centerx') - w / 2;
                };
            }

            if (tipAlign == _.L) {
                _.tipX = function(w, h) {
                    return _.x - _.get('value_space') - w;
                };
            } else if (tipAlign == _.R) {
                _.tipX = function(w, h) {
                    return _.x + _.width + _.get('value_space');
                };
            } else if (tipAlign == _.B) {
                _.tipY = function(w, h) {
                    return _.y + _.height + 3;
                };
            } else {
                _.tipY = function(w, h) {
                    return _.y - h - 3;
 ...
 */
    ...
    $.Rectangle3D = $.extend($.Rectangle, {
        ...
        drawRectangle: function() {
            var _ = this._();
            _.T.cube(_.get(_.X), _.get(_.Y), _.get('xAngle_'), _.get('yAngle_'), _.get(_.W), _.get(_.H), _.get('zHeight'), _.get('f_color'), _.get('border.enable'), _.get('border.width'), _.get('light_color'), _.get('shadow'));
        },
        isEventValid: function(e, _) {
            return {
                valid: e.x > _.x && e.x < (_.x + _.get(_.W)) && e.y < _.y + _.get(_.H) && e.y > _.y
         ...
         */
            ...
            function(a, b) {
                if (b == event) {
                    ...
                    if (f && g) _.label.doLayout(_.get('inc_x') * (_.expanded ? 1 : -1), -_.get('inc_y') * (_.expanded ? 1 : -1), 2, _.label);
                }
                return true;
            });
    ...
    $.Sector2D = $.extend($.Sector, {
        ...
        isEventValid: function(e, _) {
            if (!_.get('ignored')) {
                ...
                var r = $.distanceP2P(_.x, _.y, e.x, e.y),
                b = _.get('donutwidth');
                if (_.r < r || (b && (_.r - b) > r)) {
                    return {
                        valid: false
        ...
        tipInvoke: function() {
            ...
            Q = $.quadrantd(A);
            return function(w, h) {
                var P = $.p2Point(_.x, _.y, A, _.r * 0.8) return {
        ...
        doConfig: function() {
            ...
            var A = _.get('middleAngle'),
            L = _.pushIf('increment', $.lowTo(5, _.r / 10)),
            p2;
 ...
 */
    ...
    $.Sector3D = $.extend($.Sector, {
        ...
        doConfig: function() {
            ...
            var _ = this._(),
            ccw = _.get('counterclockwise'),
            mA = _.get('middleAngle');
 ...
 */
     ...
     */
        ...
        doParse: function(_, d, i) {
            ...
            if (_.get('sub_option.label')) _.push('sub_option.label.text', t);

            _.push('sub_option.listeners.changed',
            function(se, st, i) {
        ...
        localizer: function(_) {
            if (_.get('intellectLayout')) {
                var unlayout = [],
                layouted = [],
                d = _.get('layout_distance'),
                Q,
                x,
                y;

                _.sectors.each(function(f, i) {
                    if (f.isLabel()) unlayout.push(f.label);
                });
        ...
        doConfig: function() {
            ...
            var _ = this._(),
            r = _.get('radius'),
            f = _.get('sub_option.label') ? 0.35 : 0.44,
            pi2 = Math.PI * 2;
            ...
            var L = _.data.length,
            sepa = $.angle2Radian($.between(0, 90, _.get('separate_angle'))),
            PI = pi2 - sepa,
            sepa = sepa / L,
            eA = _.oA + sepa,
            sA = eA;
 ...
 */
     ...
     */
         ...
         */
            _.push('sub_option.radius', _.r) _.parse(_);
 ...
 */
     ...
     */
         ...
         * quick config to all rectangle
         */
            _.push('sub_option.radius', r) if (_.get(d) > 0) {
                if (_.get(d) < 1) {
 ...
 */
     ...
     */
         ...
         */
            ...
            function() {
                var L = _.data.length,
                W = _.get('coordinate.valid_width'),
                w_,
                hw,
                KL;
 ...
 */
    $.LineSegment = $.extend($.Component, {
        ...
        PP: function(_, p, x1, y1, x2, y2) {
            if (_.get('area')) {
                _.polygons.push([_.get('area_color') || _.get('light_color2'), 0, _.get('brushsize'), 0, 0, _.get('area_opacity'), _.get('smooth') ? p: [{
                    x: x1,
                    y: y1
                }].concat(p.concat([{
                    x: x2,
                    y: y2
                }])), _.get('smooth'), _.get('smoothing') || 1.5, [{
                    x: x1,
                    y: y1
                },
                {
        ...
        parse: function(_) {
            ...
            if (I) {
                var f = _.getPlugin('sign'),
                g = b,
                j = _.get('hollow_color');
        ...
        doConfig: function() {
            ...
            var _ = this._(),
            ps = _.get('point_size') * 3 / 2,
            sp = _.get('point_space'),
         ...
         */
            _.isEventValid = function(e) {
                ...
                if (ii < 0 || ii >= (p.length - 1)) {
                    ...
                    if (valid(p[ii], e.x, e.y)) return to(ii);
                    else return {
                        valid: k
                    };
                }
             ...
             */
                for (var i = ii; i <= ii + 1; i++) {
                    if (valid(p[i], e.x, e.y)) return to(i);
                }
                return {
                    valid: k
                };
 ...
 */
     ...
     */
         ...
         */
            ...
            var vw = _.coo.get('valid_width'),
            vh = _.coo.get('valid_height'),
            M = vw / (_.get('maxItemSize') - 1),
            ps = _.get('point_space');
            ...
            if (_.get('tip.enable')) {
                if ($.isFunction(_.get('tipMocker'))) {
                    ...
                    _.push('tip.invokeOffsetDynamic', true);
                    var U, x = _.coo.get(_.X),
                    y = _.coo.get(_.Y),
                    H = _.coo.get(_.H),
                    f = _.get('tipMockerOffset'),
                    r0,
                    r,
                    r1;
                    f = $.isNumber(f) ? (f < 0 ? 0 : (f > 1 ? 1 : f)) : null;
                    _.push('tip.invokeOffset',
                    function(w, h, m) {
                        if (f != null) {
                            m.top = y + (H - h) * f;
                        } else {
                 ...
                 */
                    ...
                    if (p) delete _.get('tip.listeners').parseText;
 ...
 */
     ...
     */
         ...
         */
            var S, H = _.coo.get('valid_height'),
            sp = _.get('point_space'),
            points,
            x,
            y,
